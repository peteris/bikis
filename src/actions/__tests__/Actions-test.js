...
describe('async actions', () => {
    ...
    const expectedActions = [
      {
        ...
        data: { name: 'Ride 01', distance: 100, date: '2016-01-01T00:00:00Z' },
      },
    ];
    const store = mockStore({ cycling: {} });

    return store.dispatch(actions.asyncFetchCyclingData()).then(() => {
  ...
  it('creates FETCH_SITE_CONTENT when fetching and receiving site content', () => {
    ...
    return store.dispatch(actions.asyncFetchSiteContent()).then(() => {
      expect(store.getActions()).toEqual(expectedActions);
    });
  });
  ...
  it('creates FETCH_INSTAGRAM_PHOTOS when fetching and receiving photos', () => {
    ...
    return store.dispatch(actions.asyncFetchInstagramPhotos()).then(() => {
      expect(store.getActions()).toEqual(expectedActions);
    });
  });
});
